"""
该文件用于编写基于无序点集的集合注意力基线 (SetTransSAC)
"""
import torch
...
class PMA(nn.Module):
    ...
    def forward(self, X: torch.Tensor) -> torch.Tensor:
        # X: [batchSize, seqLength, dim]
        S = self.S.repeat(X.size(0), 1, 1)  # [batchSize, num_seeds, dim]
        # Query is S, Key/Value is X
        out, _ = self.mha(S, X, X)
        return out.squeeze(1)  # [batchSize, dim]


class SetTransRadarTransformer(nn.Module):
    """
    Set Transformer (基于无序点集的 Transformer) 
    无位置编码，使用 PMA 进行聚合，用作对比基线
    """
    def __init__(self, inputDim: int=512, featureDim: int=64, headNum: int=8, layerNum: int=6, outputDim: int=32) -> None:
        ...
        # 移除位置编码器以保证置换不变性 (Permutation-Invariance)
        ...
        # 使用 PMA (Pooling by Multihead Attention) 替代普通平均池化或自定义空间注意力
        self.pooling = PMA(dim=featureDim, num_heads=headNum)
    ...
    def forward(self, radarData: torch.Tensor) -> torch.Tensor:
        x = radarData.unsqueeze(-1)  
        x = self.dimReduction(x)  
        # x = self.positionEncoder(x) # 移除位置编码
        x = self.transformerEncoder(x)  
        
        # 使用 PMA 聚合无序特征集
        x = self.pooling(x)  # [batchSize, featureDim]
...
class Actor(nn.Module):
    ...
    def __init__(self, stateDim: int=7, radarDim: int=512, actionDim: int=3, actionBound: int=2.0) -> None:
        ...
        self.radarTransformer = SetTransRadarTransformer(inputDim=radarDim, featureDim=64, outputDim=32)
        # 特征融合
...
class SetTransSAC(BaseNavigationAlgorithm):
    """
    SetTransSAC
    """
    def __init__(self, cfg) -> None:
        ...
        """
        super(SetTransSAC, self).__init__(cfg)
