"""
该文件用于编写常规多头注意力网络基线 (PointTransSAC)
"""
import torch
...
class LearnablePositionalEncoding(nn.Module):
    """
    标准的课学习位置编码 (Learnable Positional Encoding)
    用作对比传统 Transformer 与环形 Transformer 的差异
    """
    def __init__(self, embeddingDim: int=32, maxLength: int=512) -> None:
        super().__init__()
        self.positionEncoding = nn.Parameter(torch.randn(1, maxLength, embeddingDim))
...
class MHSARadarTransformer(nn.Module):
    """
    基础版多头自注意力 Transformer 网络 (MHSA)
    无环形编码和空间注意力机制，作为基线对比组
    """
    def __init__(self, inputDim: int=512, featureDim: int=64, headNum: int=8, layerNum: int=6, outputDim: int=32) -> None:
        ...
        # 用标准的可学习位置编码取代环形编码
        self.positionEncoder = LearnablePositionalEncoding(embeddingDim=featureDim, maxLength=inputDim)
    ...
    def forward(self, radarData: torch.Tensor) -> torch.Tensor:
        ...
        # 使用普通的特征融合方法: 全局平均池化 (Global Average Pooling)
        x = torch.mean(x, dim=1)  # [batchSize, featureDim]
...
class Actor(nn.Module):
    ...
    def __init__(self, stateDim: int=7, radarDim: int=512, actionDim: int=3, actionBound: int=2.0) -> None:
        ...
        self.radarTransformer = MHSARadarTransformer(inputDim=radarDim, featureDim=64, outputDim=32)
        # 特征融合
...
class MHSASAC(BaseNavigationAlgorithm):
    """
    MHSASAC
    """
    def __init__(self, cfg) -> None:
        ...
        """
        super(MHSASAC, self).__init__(cfg)
