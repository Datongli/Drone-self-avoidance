"""
该文件用于进行消融实验，用于评估不加入门控机制和Transformer机制对SAC算法的影响，相当于原版的SAC算法
"""
import torch
...
class RadarMLP(nn.Module):
    ...
    处理雷达数据的全连接网络
    """
    def __init__(self, inputDim: int=512, hiddenDim: int=256, outputDim: int=32) -> None:
        """
        处理雷达数据的全连接网络初始化函数
    ...
    def forward(self, radarData: torch.Tensor) -> torch.Tensor:
        """
        前向传播函数
        :param radarData: 输入张量，形状为[batchSize, radarDim]
        :return: 输出张量，形状为[batchSize, outputDim]
        """
        return self.net(radarData)
...
class Actor(nn.Module):
    ...
    def __init__(self, stateDim: int=7, radarDim: int=512, actionDim: int=3, actionBound: int=2.0) -> None:
        ...
        self.radarNet = RadarMLP(inputDim=radarDim, hiddenDim=256, outputDim=32)
        # 策略网络
    ...
    def forward(self, stateData: torch.Tensor, radarData: torch.Tensor, difficultyLevel: float) -> tuple:
        ...
        radarFeatures = self.radarNet(radarData)  # [batchSize, 32]
        fusedFeatures = torch.cat([stateFeatures, radarFeatures], dim=1)  # [batchSize, 64]
        """策略网络"""
...
class Critic(nn.Module):
    ...
    def forward(self, stateData: torch.Tensor, radarData: torch.Tensor, action: torch.Tensor, difficultyLevel: float) -> torch.Tensor:
        ...
        radarFeatures = self.radarNet(radarData)
        fusedFeatures = torch.cat([stateFeatures, radarFeatures], dim=1)
        # 拼接动作
...
class MTransSACWithoutTransformerAndGate(BaseNavigationAlgorithm):
    """
    MTransSAC算法实现
    """
    def __init__(self, cfg) -> None:
        ...
        """
        super(MTransSACWithoutTransformerAndGate, self).__init__(cfg)
