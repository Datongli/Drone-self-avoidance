...
class DPRLCNNProcessor(nn.Module):
    """
    DPRL 中的 1D-CNN 雷达特征提取网络
    处理一维雷达激光测距数据
    """
    def __init__(self, inputDim: int=512, featureDim: int=64, outputDim: int=32) -> None:
        super().__init__()
        # 利用 1D 卷积对雷达序列进行局部空间特征提取
        # 输入维度: [batchSize, 1, 512]
        ...
        self.conv3 = nn.Conv1d(in_channels=32, out_channels=featureDim, kernel_size=3, stride=2, padding=1)
        self.pool3 = nn.MaxPool1d(kernel_size=2)
        
        # 经过三次降采样：512 -> 256 -> 128 -> 64 -> 32 -> 16 -> 8
        self.flatten = nn.Flatten()
        
        # 全连接输出到指定维度
        self.fc = nn.Sequential(
            nn.Linear(featureDim * 8, 128),
            nn.ReLU(),
            nn.Linear(128, outputDim)
        )

    def forward(self, radarData: torch.Tensor) -> torch.Tensor:
        # 增加通道维度 [batchSize, 512] -> [batchSize, 1, 512]
...
class Actor(nn.Module):
    ...
    def __init__(self, stateDim: int=7, radarDim: int=512, actionDim: int=3, actionBound: int=2.0) -> None:
        ...
        self.radarTransformer = DPRLCNNProcessor(inputDim=radarDim, featureDim=64, outputDim=32)
        # 特征融合
...
class DPRLSAC(BaseNavigationAlgorithm):
    """
    DPRL算法实现
    """
    def __init__(self, cfg) -> None:
        ...
        """
        super(DPRLSAC, self).__init__(cfg)
