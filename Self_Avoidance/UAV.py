...
class UAV:
            ...
            for j in range(-2, 3):
                for k in range(-2, 3):
                    ...
                    if i == 1 and j == 0 and k == 0:
                        continue
                    # 判断周围是否有边界
                    if (self.x + i <= self.env.action_area[0][0] or self.x + i >= self.env.action_area[1][0]
                            or self.y + j <= self.env.action_area[0][1] or self.y + j >= self.env.action_area[1][1]
                    ...
                    for building in self.env.bds:
                        if (building.left_down[0] <= self.x + i <= building.right_up[0]
    ...
    def update(self, action):
        ...
        if dx * dx + dy * dy != 0:
            self.dir = math.acos(dx / math.sqrt(dx * dx + dy * dy)) * self.flag
        """计算能耗与相关奖励"""
        # 更新能量损耗状态
